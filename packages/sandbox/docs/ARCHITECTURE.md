...
The Sandbox SDK follows the layered architecture shared by all Frontal SDK packages:

```
┌─────────────────────────────────────┐
│  SandboxService / Namespaces         │  ← Business logic
├─────────────────────────────────────┤
...
The `SandboxService` class does NOT extend `FrontalClient` or `HttpClient`. Instead, it receives an `HttpClient` via constructor injection:
...
```typescript
// Explicit creation
const client = createSandboxClient({ apiKey: "..." });
