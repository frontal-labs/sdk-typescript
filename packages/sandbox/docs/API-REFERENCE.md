...
Complete API reference for `@frontal-labs/sandbox`.
...
```typescript
function createSandboxClient(client: FrontalClient): SandboxService;
function createSandboxClient(config: SandboxClientConfig): SandboxService;
```

Creates a new Sandbox service instance. Accepts either a shared `FrontalClient` (recommended) or a standalone configuration object.
...
`SandboxService` — A new service instance.
