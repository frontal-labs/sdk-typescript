...
In-depth guide for using `@frontal-labs/billing` in production.
...
const { http, mock } = createTestHttpClient([
  { method: "GET", path: "/v1/billing/resource", body: { data: [...] } }
]);

const service = new BillingService(http);
const result = await service.someMethod();

mock.expectCalled("GET", "/v1/billing/resource");
```
