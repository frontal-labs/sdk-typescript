# Lineage Developer Guide

In-depth guide for using `@frontal-labs/lineage` in production.
...
const { http, mock } = createTestHttpClient([
  { method: "GET", path: "/v1/lineage/resource", body: { data: [...] } }
]);

const service = new LineageService(http);
const result = await service.someMethod();

mock.expectCalled("GET", "/v1/lineage/resource");
```
