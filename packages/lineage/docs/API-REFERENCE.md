...
Complete API reference for `@frontal-labs/lineage`.
...
```typescript
function createLineageClient(client: FrontalClient): LineageService;
function createLineageClient(config: LineageClientConfig): LineageService;
```

Creates a new Lineage service instance. Accepts either a shared `FrontalClient` (recommended) or a standalone configuration object.
...
`LineageService` — A new service instance.
