# Vectors Developer Guide

In-depth guide for using `@frontal-labs/vectors` in production.
...
const { http, mock } = createTestHttpClient([
  { method: "GET", path: "/v1/vectors/resource", body: { data: [...] } }
]);

const service = new VectorsService(http);
const result = await service.someMethod();

mock.expectCalled("GET", "/v1/vectors/resource");
```
