...
In-depth guide for using `@frontal-labs/schedules` in production.
...
const { http, mock } = createTestHttpClient([
  { method: "GET", path: "/v1/schedules/resource", body: { data: [...] } }
]);

const service = new SchedulesService(http);
const result = await service.someMethod();

mock.expectCalled("GET", "/v1/schedules/resource");
```
