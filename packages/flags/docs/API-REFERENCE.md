...
Complete API reference for `@frontal-labs/flags`.
...
```typescript
function createFlagsClient(client: FrontalClient): FlagsService;
function createFlagsClient(config: FlagsClientConfig): FlagsService;
```

Creates a new Flags service instance. Accepts either a shared `FrontalClient` (recommended) or a standalone configuration object.
...
`FlagsService` — A new service instance.

### Default Singleton

```typescript
import { flags } from "@frontal-labs/flags";
```
