# Queues SDK Architecture

## System Design

The Queues SDK follows the layered architecture shared by all Frontal SDK packages:

```
┌─────────────────────────────────────┐
│  QueuesService / Namespaces         │  ← Business logic
├─────────────────────────────────────┤
...
The `QueuesService` class does NOT extend `FrontalClient` or `HttpClient`. Instead, it receives an `HttpClient` via constructor injection:
...
```typescript
// Explicit creation
const client = createQueuesClient({ apiKey: "..." });
