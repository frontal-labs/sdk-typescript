# Queues Developer Guide

In-depth guide for using `@frontal-labs/queues` in production.
...
const { http, mock } = createTestHttpClient([
  { method: "GET", path: "/v1/queues/resource", body: { data: [...] } }
]);

const service = new QueuesService(http);
const result = await service.someMethod();

mock.expectCalled("GET", "/v1/queues/resource");
```
