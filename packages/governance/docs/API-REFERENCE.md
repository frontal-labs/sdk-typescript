...
Complete API reference for `@frontal-labs/governance`.
...
```typescript
function createGovernanceClient(client: FrontalClient): GovernanceService;
function createGovernanceClient(config: GovernanceClientConfig): GovernanceService;
```

Creates a new Governance service instance. Accepts either a shared `FrontalClient` (recommended) or a standalone configuration object.
...
`GovernanceService` — A new service instance.
