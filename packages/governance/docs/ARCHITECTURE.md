# Governance SDK Architecture

## System Design

The Governance SDK follows the layered architecture shared by all Frontal SDK packages:
...
The `GovernanceService` class does NOT extend `FrontalClient` or `HttpClient`. Instead, it receives an `HttpClient` via constructor injection:
...
```typescript
// Explicit creation
const client = createGovernanceClient({ apiKey: "..." });
