...
In-depth guide for using `@frontal-labs/governance` in production.
...
const { http, mock } = createTestHttpClient([
  { method: "GET", path: "/v1/governance/resource", body: { data: [...] } }
]);

const service = new GovernanceService(http);
const result = await service.someMethod();

mock.expectCalled("GET", "/v1/governance/resource");
```
