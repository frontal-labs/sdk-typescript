# Datasets Developer Guide

In-depth guide for using `@frontal-labs/datasets` in production.
...
```typescript
import { CircuitBreaker } from "frontal/core";
...
```typescript
import { RateLimitError } from "frontal/core";
...
Use `frontal/testing` to mock HTTP:

```typescript
import { createTestHttpClient } from "frontal/testing";

const { http, mock } = createTestHttpClient([
  { method: "GET", path: "/v1/datasets/resource", body: { data: [...] } }
]);

const service = new DatasetsService(http);
const result = await service.someMethod();

mock.expectCalled("GET", "/v1/datasets/resource");
```
