# Datasets SDK Architecture

## System Design

The Datasets SDK follows the layered architecture shared by all Frontal SDK packages:
...
The `DatasetsService` class does NOT extend `FrontalClient` or `HttpClient`. Instead, it receives an `HttpClient` via constructor injection:
...
```typescript
// Explicit creation
const client = createDatasetsClient({ apiKey: "..." });
