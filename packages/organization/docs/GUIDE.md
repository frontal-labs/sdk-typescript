# Organization Developer Guide

In-depth guide for using `@frontal-labs/organization` in production.
...
const { http, mock } = createTestHttpClient([
  { method: "GET", path: "/v1/organization/resource", body: { data: [...] } }
]);

const service = new OrganizationService(http);
const result = await service.someMethod();

mock.expectCalled("GET", "/v1/organization/resource");
```
