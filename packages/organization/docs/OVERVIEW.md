# Organization SDK Overview

The `@frontal-labs/organization` package provides a TypeScript/JavaScript client for Frontal's Multi-tenancy: tenants, teams, members, roles.

## Installation

```bash
bun add @frontal-labs/organization
# or
npm install @frontal-labs/organization
```

## Quick Start

```typescript
import { createOrganizationClient } from "@frontal-labs/organization";

const client = createOrganizationClient({
  apiKey: process.env.FRONTAL_API_KEY!
});

// Use the client...
```

## Environment Variables

| Variable | Required | Default | Description |
|---|---|---|---|
| `FRONTAL_API_KEY` | Yes | — | Your Frontal API key |
| `FRONTAL_ORGANIZATION_API_URL` | No | `https://api.frontal.dev/v1` | Custom API base URL |
| `NODE_ENV` | No | `development` | Runtime environment |

## Core Concepts

The Organization SDK follows the composition-over-inheritance pattern used across all Frontal SDKs. The service class accepts an `HttpClient` from `@frontal-labs/core` for transport, uses Zod schemas for runtime validation, and provides both a standalone factory function (`createOrganizationClient`) and a default singleton (`organization`).

## Configuration

### Standalone Client

```typescript
import { createOrganizationClient } from "@frontal-labs/organization";

const client = createOrganizationClient({
  apiKey: "frt_...",
  baseUrl: "https://api.frontal.dev/v1",
  timeout: 30000,
  maxRetries: 3
});
```

### Shared FrontalClient (recommended)

```typescript
import { FrontalClient } from "@frontal-labs/core";
import { createOrganizationClient } from "@frontal-labs/organization";

const frontal = new FrontalClient({ apiKey: process.env.FRONTAL_API_KEY! });
const client = createOrganizationClient(frontal);
```

## Use Cases

1. Multi-tenancy: tenants
2. teams
3. members
4. roles

## Error Handling

The Organization SDK throws typed errors from `@frontal-labs/core`:

```typescript
import { FrontalError, RateLimitError, NotFoundError } from "@frontal-labs/core";

try {
  await client.someMethod();
} catch (error) {
  if (error instanceof NotFoundError) {
    console.error("Resource not found:", error.message);
  } else if (error instanceof RateLimitError) {
    console.error("Rate limited — retry after", error.retryAfter, "s");
  } else if (error instanceof FrontalError) {
    console.error("API error:", error.code, error.statusCode);
  }
}
```

## Related Packages

- [@frontal-labs/core](../core/docs/OVERVIEW.md)
- [@frontal-labs/auth](../auth/docs/OVERVIEW.md)
- [@frontal-labs/governance](../governance/docs/OVERVIEW.md)
- [@frontal-labs/billing](../billing/docs/OVERVIEW.md)
