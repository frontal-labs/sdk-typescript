# Organization SDK Architecture

## System Design

The Organization SDK follows the layered architecture shared by all Frontal SDK packages:

```
┌─────────────────────────────────────┐
│  OrganizationService / Namespaces         │  ← Business logic
├─────────────────────────────────────┤
...
The `OrganizationService` class does NOT extend `FrontalClient` or `HttpClient`. Instead, it receives an `HttpClient` via constructor injection:
...
```typescript
// Explicit creation
const client = createOrganizationClient({ apiKey: "..." });
