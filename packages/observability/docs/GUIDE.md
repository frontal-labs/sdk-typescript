...
In-depth guide for using `@frontal-labs/observability` in production.
...
const { http, mock } = createTestHttpClient([
  { method: "GET", path: "/v1/observability/resource", body: { data: [...] } }
]);

const service = new ObservabilityService(http);
const result = await service.someMethod();

mock.expectCalled("GET", "/v1/observability/resource");
```
