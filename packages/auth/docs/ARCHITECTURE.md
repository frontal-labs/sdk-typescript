# Auth SDK Architecture

## System Design

The Auth SDK follows the layered architecture shared by all Frontal SDK packages:
...
The `AuthService` class does NOT extend `FrontalClient` or `HttpClient`. Instead, it receives an `HttpClient` via constructor injection:
...
```typescript
// Explicit creation
const client = createAuthClient({ apiKey: "..." });
