...
In-depth guide for using `@frontal-labs/audit` in production.
...
const { http, mock } = createTestHttpClient([
  { method: "GET", path: "/v1/audit/resource", body: { data: [...] } }
]);

const service = new AuditService(http);
const result = await service.someMethod();

mock.expectCalled("GET", "/v1/audit/resource");
```
