---
"@frontal-labs/auth": major
---

Align the auth client with the real `iam/backend/auth-server` routes and build
out the account + admin surfaces.

- **Path fixes** (previously mis-routed):
  - Admin operations now use `/auth/admin/*` (were `/admin/*`, which the gateway
    would not route under its `/v1/auth` prefix).
  - Session APIs use `/auth/auth/session` (the gateway maps `/v1/auth/auth/*` →
    the service's `/api/auth/*`); was `/auth/session`.
  - MFA maps to the real factors API: `enroll` → `POST /auth/factors`, `challenge`
